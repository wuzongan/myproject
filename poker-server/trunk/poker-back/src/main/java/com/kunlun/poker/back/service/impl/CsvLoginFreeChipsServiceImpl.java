...
public class CsvLoginFreeChipsServiceImpl implements CsvLoginFreeChipsService {
    ...
    private int endTime;
    private Map<Integer, CsvLoginFreeChipsLog> dataMap;
    
    @Override
    public  Map<Integer, CsvLoginFreeChipsLog> obtainLogDataInFile(int startTime, int endTime, 
        ...
        if(this.startTime == 0 && this.endTime == 0 && dataMap == null){
            ...
            /** 支持分页*/
            return PageUtil.getPageMap(dataMap, new TreeMap<Integer, CsvLoginFreeChipsLog>(), pageSize, pageIndex);
        }
...
//        Config.getInstance().getLogDirName()
                ...
                try(RandomAccessFile raf = new RandomAccessFile(startTimePath, "r");){
                    ...
                    if(startLine != -1 && endLine != -1){
                        ...
                        if(totalLine > Constants.READ_LINE_COUNT){
                            Map<Integer, CsvLoginFreeChipsLog> sourceMap = new TreeMap<Integer, CsvLoginFreeChipsLog>();
            ...
            for(int idx=0; idx<diffValue; idx++){
                ...
                try(RandomAccessFile raf = new RandomAccessFile(startTimePath, "r");) {
                    ...
                    if(startLine != -1 && endLine != -1){
                        int totalLine = endLine - startLine;
                        if(totalLine > Constants.READ_LINE_COUNT){
                            Map<Integer, CsvLoginFreeChipsLog> oneHSourceMap = new TreeMap<Integer, CsvLoginFreeChipsLog>();
                            int size = totalLine % Constants.READ_LINE_COUNT==0 ? totalLine/Constants.READ_LINE_COUNT : totalLine/Constants.READ_LINE_COUNT + 1;
                            int tempEndLine = startLine;
                            do {
                            	String dataStr = FileUtil.obtainStringInRandomAccessFile(mbb, startLine, tempEndLine, LINE_BYTE_LENGTH);
	...
	public <E extends Number,T extends AbstactLog<E>> Map<E, T> transform(String str){
        Map<Integer, CsvLoginFreeChipsLog> map = new TreeMap<Integer, CsvLoginFreeChipsLog>();
        String[] arr = str.split("\n");
        for(String o : arr){
            String[] log = o.split(",");
            int userId = Integer.valueOf(log[0].trim());
            long loginFreeChips = Long.parseLong(log[1].trim());
            int time = Integer.valueOf(log[2].trim().replace("\r", ""));
            
        	int logId = userId;
        	System.out.println(logId);
            
        	CsvLoginFreeChipsLog logModel = map.get(logId);
            if(logModel == null){
                logModel = new CsvLoginFreeChipsLog();
            }
            logModel.setId(logId);
            logModel.setLoginFreeChips(loginFreeChips);
    ...
    public <E extends Number, T extends AbstactLog<E>> Map<E, T> merge(Map<E, T> sMap, Map<E, T> tMap){
		Map<Integer, CsvLoginFreeChipsLog> sourceMap = (Map<Integer, CsvLoginFreeChipsLog>) sMap;
        Map<Integer, CsvLoginFreeChipsLog> targetMap = (Map<Integer, CsvLoginFreeChipsLog>) tMap;
        for(Map.Entry<Integer, CsvLoginFreeChipsLog> entry : targetMap.entrySet()){
        	Integer Id = entry.getKey();
            CsvLoginFreeChipsLog sourceLog = sourceMap.get(Id);
            CsvLoginFreeChipsLog targetLog = entry.getValue();
            
            if(sourceLog == null){
                sourceLog = new CsvLoginFreeChipsLog();
    ...
    public static void main(String[] args) throws Exception {
        ...
        CsvLoginFreeChipsServiceImpl csv = new CsvLoginFreeChipsServiceImpl();
        Map<Integer, CsvLoginFreeChipsLog> getCsv = csv.obtainLogDataInFile(1412784001, 1412784100, 10, 0);
