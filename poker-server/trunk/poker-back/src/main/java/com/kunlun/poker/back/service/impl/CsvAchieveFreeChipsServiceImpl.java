...
import com.kunlun.poker.back.service.CsvAchieveFreeChipsService;
 ...
 *
 */
@Service("csvAchieveFreeChipsService")
public class CsvAchieveFreeChipsServiceImpl implements CsvAchieveFreeChipsService {
    ...
    private int endTime;
    private Map<Integer, CsvAchieveFreeChipsLog> dataMap;
    
    @Override
    public  Map<Integer, CsvAchieveFreeChipsLog> obtainLogDataInFile(int startTime, int endTime, 
        ...
        if(this.startTime == 0 && this.endTime == 0 && dataMap == null){
            ...
            /** 支持分页*/
            return PageUtil.getPageMap(dataMap, new TreeMap<Integer, CsvAchieveFreeChipsLog>(), pageSize, pageIndex);
        }
...
//        Config.getInstance().getLogDirName()
        String startTimePath = String.format("%s%s_%s", "D:\\workcode\\poker-server\\trunk\\poker-log\\log" + File.separator,
                LogConstants.TL_CSV_ACHIEVEMENT_FREE_CHIPS_STATIC, startTimeStr+".csv");
                ...
                try(RandomAccessFile raf = new RandomAccessFile(startTimePath, "r");){
                    ...
                    if(startLine != -1 && endLine != -1){
                        ...
                        if(totalLine > Constants.READ_LINE_COUNT){
                            Map<Integer, CsvAchieveFreeChipsLog> sourceMap = new TreeMap<Integer, CsvAchieveFreeChipsLog>();
            ...
            for(int idx=0; idx<diffValue; idx++){
                ...
                try(RandomAccessFile raf = new RandomAccessFile(startTimePath, "r");) {
                    ...
                    if(startLine != -1 && endLine != -1){
                        int totalLine = endLine - startLine;
                        if(totalLine > Constants.READ_LINE_COUNT){
                            Map<Integer, CsvAchieveFreeChipsLog> oneHSourceMap = new TreeMap<Integer, CsvAchieveFreeChipsLog>();
                            int size = totalLine % Constants.READ_LINE_COUNT==0 ? totalLine/Constants.READ_LINE_COUNT : totalLine/Constants.READ_LINE_COUNT + 1;
                            int tempEndLine = startLine;
                            do {
                            	String dataStr = FileUtil.obtainStringInRandomAccessFile(mbb, startLine, tempEndLine, LINE_BYTE_LENGTH);
	...
	public <E extends Number,T extends AbstactLog<E>> Map<E, T> transform(String str){
        Map<Integer, CsvAchieveFreeChipsLog> map = new TreeMap<Integer, CsvAchieveFreeChipsLog>();
        String[] arr = str.split("\n");
        for(String o : arr){
            String[] log = o.split(",");
            int userId = Integer.valueOf(log[0].trim());
            long achieveFreeChips = Long.parseLong(log[1].trim());
            int time = Integer.valueOf(log[2].trim().replace("\r", ""));
            
        	int logId = userId;
        	System.out.println(logId);
            
        	CsvAchieveFreeChipsLog logModel = map.get(logId);
            if(logModel == null){
                logModel = new CsvAchieveFreeChipsLog();
            }
            logModel.setId(logId);
            logModel.setAchievementFreeChips(achieveFreeChips);
    ...
    public <E extends Number, T extends AbstactLog<E>> Map<E, T> merge(Map<E, T> sMap, Map<E, T> tMap){
		Map<Integer, CsvAchieveFreeChipsLog> sourceMap = (Map<Integer, CsvAchieveFreeChipsLog>) sMap;
        Map<Integer, CsvAchieveFreeChipsLog> targetMap = (Map<Integer, CsvAchieveFreeChipsLog>) tMap;
        for(Map.Entry<Integer, CsvAchieveFreeChipsLog> entry : targetMap.entrySet()){
        	Integer Id = entry.getKey();
        	CsvAchieveFreeChipsLog sourceLog = sourceMap.get(Id);
        	CsvAchieveFreeChipsLog targetLog = entry.getValue();
            
            if(sourceLog == null){
                sourceLog = new CsvAchieveFreeChipsLog();
    ...
    public static void main(String[] args) throws Exception {
        RandomAccessFile file = new RandomAccessFile(
                "D:\\workcode\\poker-server\\trunk\\poker-log\\log\\tl_csv_achievement_free_chips_statics_2014_10_09.csv",
                "r");
        ...
        int startLine = FileUtil.binarySearch(mbb, 1412784000, lineCount, CLOUMN_COUNT, 44, 8);
        ...
        Map<Integer, CsvAchieveFreeChipsLog> getCsv = csv.obtainLogDataInFile(1412784001, 1412784100, 10, 0);
