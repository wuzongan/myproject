...
local SOCKET_RECONNECT_TIME = 2			-- socket reconnect try interval
local SOCKET_CONNECT_FAIL_TIMEOUT = 3	-- socket failure timeout
local SOCKET_RECONNECT_FAIL_CONUT = 2	-- socket reconnect failure conut
...
function SocketTCP:ctor(__host, __port, __retryConnectWhenFailure)
	...
	self.immediatelyReConnect = true
end

function SocketTCP:setImmediatelyReConnect( __immediately )
	self.immediatelyReConnect = __immediately
	return self
end
...
function SocketTCP:setReconnCount(count)
	SOCKET_RECONNECT_FAIL_CONUT = count
	return self
end
	...
	if not __checkConnect() then
		...
		local __connectTimeTick = function ()
			...
			if self.waitConnect >= SOCKET_CONNECT_FAIL_TIMEOUT then
				print("======connect================",self.waitConnect)
...
function SocketTCP:_reconnect()
	...
	local __doReConnect = function ()
		self.reconnect_conut = self.reconnect_conut or 0
		self.reconnect_conut = self.reconnect_conut + 1
		if self.reconnect_conut > SOCKET_RECONNECT_FAIL_CONUT then
	...
	if self.immediatelyReConnect then 
		__doReConnect() 
	else
...
function SocketTCP:reconnect_failure()
	self.reconnect_conut = nil
	if self.reconnectScheduler then scheduler.unscheduleGlobal(self.reconnectScheduler) end
