...
//add by xiaolinfu
void CCParticleSystemQuadLoader::onHandlePropTypeTexture2(CCNode * pNode, CCNode * pParent, const char * pPropertyName, CCTexture2D * pCCTexture2D, CCRect &rect, CCBReader * pCCBReader) {
    if(strcmp(pPropertyName, PROPERTY_TEXTURE) == 0) {
        ((CCParticleSystemQuad *)pNode)->setTextureWithRect(pCCTexture2D,rect);
    } else {
        CCNodeLoader::onHandlePropTypeTexture(pNode, pParent, pPropertyName, pCCTexture2D, pCCBReader);
    }
}
