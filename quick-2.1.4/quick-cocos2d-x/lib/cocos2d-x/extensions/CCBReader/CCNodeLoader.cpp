...
// #endif
    ...
    //           CCTexture2D* ccTexture2D = this->parsePropTypeTexture2(pNode, pParent, pCCBReader,rect);
			 // // CCLOG("  %f  %f",rect.size.width,rect.size.height);
    //            if(setProp) 
    //            {
    //                this->onHandlePropTypeTexture2(pNode, pParent, propertyName.c_str(), ccTexture2D, rect,pCCBReader);
    //            }
    //            break;
            }
...
CCSpriteFrame * CCNodeLoader::parsePropTypeSpriteFrame(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader, const char *pPropertyName) 
{
    ...
    {
		 //add by xiaolinfu  保持兼容 如果有打包优先选打包资源
		//if(spriteSheet.length() == 0)
		//{
		//	size_t start = spriteFile.find_last_of("/")+1;
		//	size_t dirstart = spriteFile.substr(0,start-1).find_last_of("/")+1;
		//	string plist = spriteFile.substr(0,start) + spriteFile.substr(dirstart,start-dirstart-1) + "1.plist";
		...
		//	{
		//		spriteSheet = plist;
		//		spriteFile = spriteFile.substr(start);
		//	}
		//}
		////add end
       /* if (spriteSheet.length() == 0)
...
//add by xiaolinfu
CCTexture2D * CCNodeLoader::parsePropTypeTexture2(CCNode * pNode, CCNode * pParent ,CCBReader * pCCBReader,CCRect &rect) {
    std::string spriteFile = pCCBReader->getCCBRootPath() + pCCBReader->readCachedString();
    if (spriteFile.length() > 0)
    {
		CCSpriteFrame* frame =  CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(spriteFile.c_str());
		rect = frame->getRect();
...
//add by xiaolinfu
void CCNodeLoader::onHandlePropTypeTexture2(CCNode * pNode, CCNode * pParent, const char* pPropertyName, CCTexture2D * pCCTexture2D,CCRect &rect, CCBReader * pCCBReader) {
    //ASSERT_FAIL_UNEXPECTED_PROPERTY(pPropertyName);
}
