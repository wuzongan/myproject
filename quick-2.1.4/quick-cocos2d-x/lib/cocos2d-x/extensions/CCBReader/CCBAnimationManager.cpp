...
//     }
    ...
    setDelegate(NULL);
    //update wza
    CC_SAFE_RELEASE(mAnimationCompleteCallbackFunc);
...
//update wza
void CCBAnimationManager::pauseAnimation(){
    CCDictElement* pElement = NULL;
    CCDICT_FOREACH(mNodeSequences, pElement)
    {
        CCNode *node = (CCNode*)pElement->getIntKey();
        mRootNode->pauseSchedulerAndActions();
        node->pauseSchedulerAndActions();
    }
}

void CCBAnimationManager::resumeAnimation(){
    CCDictElement* pElement = NULL;
    CCDICT_FOREACH(mNodeSequences, pElement)
    {
        CCNode *node = (CCNode*)pElement->getIntKey();
        mRootNode->resumeSchedulerAndActions();
        node->resumeSchedulerAndActions();
    }
}

void CCBAnimationManager::addParticle(CCNode *node){
    mParticles->addObject(node);
}

CCArray* CCBAnimationManager::getParticles(){
    return mParticles;
}
...
//update wza
void CCBAnimationManager::setAnimationCompletedCallback(CCObject *target, CCCallFunc* callbackFunc) {
//    if (target)
//    {
//        target->retain();
//    }
...
//    mTarget = target;
    ...
    mAnimationCompleteCallbackFunc->retain();
}
...
void CCBAnimationManager::sequenceCompleted()
{
    ...
    if (mAnimationCompleteCallbackFunc) {
//        (mTarget->*mAnimationCompleteCallbackFunc)();
        mAnimationCompleteCallbackFunc->execute();
    }
...
{
    CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect(mSoundFile.c_str(),false,mPitch,mPan,mGain);
}
